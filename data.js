...
function renderDialogueStep(card) {
  ...
  var hist = "";
  if (dialogueHistory.length > 0) { hist = '<div style="margin-bottom:20px;padding:16px;background:#fafaf9;border-radius:var(--radius-sm);max-height:200px;overflow-y:auto;">'; dialogueHistory.forEach(function(h) { hist += '<p style="font-size:0.82rem;color:var(--muted);margin-bottom:2px;">—— 我问：</p><p style="font-size:0.88rem;margin-bottom:8px;">' + escapeHtml(h.q) + '</p><p style="font-size:0.82rem;color:var(--muted);margin-bottom:2px;">—— 你答：</p><p style="font-size:0.88rem;margin-bottom:14px;color:var(--fg2);">' + escapeHtml(h.a) + '</p>'; }); hist += '</div>'; }
...
function renderIdealTypeBox(p) {
  ...
  return '<div style="background:linear-gradient(135deg,#fdf4ff,#fffbeb);border:1.5px solid var(--accent-border);border-radius:var(--radius);padding:20px;margin-bottom:24px;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;"><span style="font-size:1.8rem;">' + pw.icon + '</span><div><span style="font-weight:700;font-size:1rem;color:var(--fg);">' + pw.name + '</span><span style="font-size:0.78rem;color:var(--muted);display:block;">' + pw.tagline + '</span></div></div><p style="font-size:0.82rem;color:var(--fg2);margin-bottom:12px;">' + pw.desc + '</p><div style="background:#fff;border:1px solid var(--green-border);border-radius:8px;padding:12px;margin-bottom:10px;"><span style="font-size:0.75rem;color:var(--green);font-weight:650;">⚡ 当前境界：' + stage.name + '（' + stage.icon + '）</span><p style="font-size:0.78rem;color:var(--fg2);margin-top:2px;">' + stage.desc + '</p></div><div style="font-size:0.73rem;color:var(--muted);">修炼路径：' + pw.stages.map(function(s,i) { return '<span style="color:' + (i <= (p.stage||0) ? 'var(--green)' : 'var(--muted)') + ';">' + s.icon + s.name + '</span>'; }).join(" → ") + '</div></div>';
}
...
function renderActionDashboard(card, p, plan) {
  ...
  if (allDone && p.idealType) {
    ...
    if (pw && (p.stage || 0) < pw.stages.length - 1) {
      ...
      breakthroughHtml = '<div style="background:linear-gradient(135deg,var(--accent-bg),#fff);border:2px solid var(--accent-border);border-radius:var(--radius);padding:18px;margin-top:16px;text-align:center;"><span style="font-size:1.6rem;display:block;margin-bottom:4px;">⚡</span><p style="font-weight:700;color:var(--accent);margin-bottom:4px;">突破境界：' + (pw.stages[p.stage||0].name + ' → ' + nextStage.name) + '</p><p style="font-size:0.82rem;color:var(--fg2);margin-bottom:10px;">' + nextStage.desc + '</p><button class="btn btn-primary btn-sm" onclick="breakthrough()">✨ 突破！</button></div>';
    }
  }
...
function breakthrough() {
  ...
  var toast = document.createElement("div"); toast.className = "copied-toast"; toast.textContent = "🎉 突破！你已晋升为 " + pw.stages[newStage].name + "！";
